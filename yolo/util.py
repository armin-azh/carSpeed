...
def write_to_image(x, img, color):
    x = x.astype(np.int)
    c1 = tuple(x[:2])
    c2 = tuple(x[2:4])
    cls = x[4]
    ...
    cv2.putText(img, label, (c1[0], c1[1] - 4), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, [225, 255, 255], 1)
    cv2.putText(img, speed, (c1[0], c1[1] + 4), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, [225, 255, 255], 1)
    return img
